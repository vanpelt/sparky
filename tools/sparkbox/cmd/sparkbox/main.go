...
func serve(args []string) error {
	...
	var (
		...
		idleBalloon          = fs.Duration("idle-balloon", 20*time.Minute, "balloon a warm sandbox down to --mem-reserve-mb after this much idle, reclaiming its RAM while it keeps running (0 disables; needs --mem-reserve-mb). Fires on the idle timer alone, whether or not a budget is under pressure; --owner-memory-pool-mb drives the separate demand-led reclaim. Two minutes was the old default and it reliably squeezed guests that had not finished booting")
