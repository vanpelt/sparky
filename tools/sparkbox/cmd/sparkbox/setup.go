...
func newSetupFlags(cfg hostsetup.Config) (*flag.FlagSet, *setupOpts) {
	...
	o.agentTools = fs.Bool("agent-tools", cfg.AgentTools, "bake the agent CLIs (claude, codex, pi, hivemind) + guest workload identity into the rootfs templates and install the daily refresher. On by default — a sandbox with no agent in it is rarely what anyone wants; --agent-tools=false leaves the templates bare")
