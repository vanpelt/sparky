...
// MACOUI is the third octet of every guest MAC a QEMU sandbox gets. It lives
// beside BalloonDeviceID and for the same reason: TWO processes build this
// argv -- the driver on the direct path, the privileged helper on the jailed
// one -- and QEMU takes the MAC from argv on the cold boot AND the restore.
// Two copies of this value drift into a sandbox that loses its network on
// RESUME, presenting the guest kernel with an interface its netcfg hook has
// never seen, long after the change that caused it.
//
// 0x01 where the firecracker driver uses 0x00, so two drivers on one host
// cannot hand the same address to two live guests on one L2 segment.
const MACOUI = 0x01
