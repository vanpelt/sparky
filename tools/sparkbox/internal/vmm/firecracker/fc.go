...
func (d *Driver) boot(ctx context.Context, name string, vcpus, memMB int64, st *vmState, rootfs string, snapshot []string, fresh bool) error {
	...
	if d.opts.JailerBin != "" {
		...
		// none of the causes. The QEMU driver has written a qemu.log from the
		// start (internal/vmm/qemu/lifecycle.go) and it is the reason its
		// failures are readable; this is the same idea, arrived at the hard way
		// after an x86_64 Reboot failure that said only "context deadline
		// exceeded". Best-effort: a boot must not fail because a log file
		// could not be opened.
	...
	if err := m.Start(vmCtx); err != nil {
		...
		// VM directory, and a caller that tears the directory down (every
		// parity fixture does) would otherwise destroy the only evidence.
		// internal/vmm/qemu does this with qemuLogTail and it is the reason its
