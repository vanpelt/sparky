...
func (d *Driver) Snapshot(ctx context.Context, name, newImage string) error {
	...
	//
	// Every per-guest identity this removes is now regenerated by the fork
	// itself, at first boot, before sshd: sparkbox-identity-reset clears the SSH
	// host keys and the dbus id when the cmdline's sandbox name disagrees with
	// the stamp on the disk, and the machine id comes from systemd.machine_id=
	// on that same cmdline. So a template is safe to boot whether or not this
	// ran — which is what lets a host that cannot mount capture one at all.
	//
...
//
// It exists because of forks. A template is a byte-for-byte copy of somebody's
// rootfs, machine-id included, and PID 1 reads that file before any unit runs —
// so a guest-side fix is correct only from a fork's SECOND boot. systemd reads
// systemd.machine_id= off the kernel command line instead, which the host
// writes per boot and no guest can forge, so a fork differs from its parent
// from the first instruction with nothing on the disk having to be edited and
// nothing in the guest having to cooperate.
//
...
//
// The guest-side clear in sparkbox-netcfg stays regardless — it covers the dbus
// copy, the SSH host keys, and images with no systemd to read this at all.
