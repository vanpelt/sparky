...
// zerofree all work on the image file — and everything sanitizeTemplate removed
// is now regenerated by the fork itself at first boot (sparkbox-identity-reset,
// plus systemd.machine_id= on the cmdline). So the capture proceeds and the one
// step that would mount is skipped.
//
