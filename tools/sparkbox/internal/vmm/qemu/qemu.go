...
// counterpart here. That is a scope decision, not an oversight: Firecracker is
// a static binary and its chroot holds four files, while the packaged QEMU is
// dynamically linked against a large library closure that would all have to be
// staged per slot. Confining QEMU is real work with a real design (CoreWeave's
// own answer is Kata), and doing it badly is worse than doing it later. Until
// then this driver is the direct launcher only, which is exactly what
// hack/parity/run-on-mac.sh already exercises.
type Options struct {
