...
// sshActivityInterval bounds activity marks generated by stream traffic. The
// manager and fleet coalesce again at ten seconds, but doing that work on every
// terminal read would still turn a busy shell into needless lock traffic.
const sshActivityInterval = time.Minute
...
func (g *Gateway) handle(s gssh.Session) {
	...
	exitCode, err := g.pipeSession(s, client, sandboxName, viaNewDoor)
	if err != nil {
...
func (g *Gateway) pipeSession(s gssh.Session, client *xssh.Client, sandbox string, tagsOnly bool) (int, error) {
	...
	activity := &streamActivity{
		interval: sshActivityInterval,
		mark:     func() { g.mgr.MarkActive(sandbox) },
	}
	go func() {
		io.Copy(stdin, activity.reader(s)) //nolint:errcheck
		stdin.Close()
	}()
	done := make(chan struct{}, 2)
	go func() { io.Copy(activity.writer(s), stdout); done <- struct{}{} }()          //nolint:errcheck
	go func() { io.Copy(activity.writer(s.Stderr()), stderr); done <- struct{}{} }() //nolint:errcheck
...
// streamActivity turns actual SSH bytes in either direction into an activity
// mark. Merely holding a shell open remains idle, so an abandoned terminal can
// still be reaped; typing, command output, and forwarded stream traffic keep
// the sandbox warm without relying on incidental VMM CPU use.
...
func (a *streamActivity) reader(r io.Reader) io.Reader {
	return activityReader{Reader: r, activity: a}
}

func (a *streamActivity) writer(w io.Writer) io.Writer {
	return activityWriter{Writer: w, activity: a}
}
