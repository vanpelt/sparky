...
func TestPageCarriesCSPAndTerminal(t *testing.T) {
	...
	for _, want := range []string{"/assets/xterm.js", "/assets/xterm.css", "/assets/addon-clipboard.js", "/assets/sparkbox-logo.png", "sparkbox.terminal.v1", "id=\"term\"", "id=\"hivemind-session\"", "id=\"proxy-link\"", "id=\"proxy-dlg\"", "listening_ports", "port_services", "window.open(location.href"} {
...
func TestAssetsAreServedImmutably(t *testing.T) {
	...
	for _, c := range []struct{ path, ctype string }{
		...
		{"/assets/addon-clipboard.js", "text/javascript"},
