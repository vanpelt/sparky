...
//
// Both directions tally bytes into `flows`, keyed by (tap ifindex, remote
// address), so userspace can attribute a flow to a specific VM (the tap is 1:1
// with a guest) and join the address back to a domain via the DNS proxy's
// table. In enforce mode, from_guest drops any packet whose (ifindex, remote)
// is not present in `allowed` — with ifindex 0 acting as an all-VMs wildcard so
// a globally-pinned infra address (or a legacy fleet-wide allow-set) reaches
// every tap. The allow-set userspace mirrors from resolved DNS answers plus
...
//
// Enforcement is gated per tap: it applies to a packet only when the packet's
// tap ifindex is present in the `enforced` set. A tap absent from that set is
// metered but never dropped — the way an untagged sandbox (one with no network
// policy) keeps unrestricted egress while a policied one is filtered. The
// master `config` switch still guards everything, so clearing it disables all
// enforcement regardless of the per-tap set.
//
...
// A flow key is (tap ifindex, remote address). The remote is always stored as a
// 16-byte IPv6 value; IPv4 is held v4-mapped (::ffff:a.b.c.d) so a single key
// type covers both families and matches Go's netip.Addr.As16(). ifindex is the
// tap the packet crossed — a stable 1:1 discriminator for the guest VM within a
// single program lifetime (a tap that is torn down and recreated across a VM
// pause/resume gets a fresh ifindex, which userspace reconciles by tap name).
// ifindex 0 never names a real tap, so the allow-set reserves it as an all-VMs
// wildcard. The layout (u32 then 16 bytes = 20 bytes, 4-byte aligned, no
// padding) matches Go's struct{ Ifindex uint32; Addr [16]byte }.
struct flow_key {
  __u32 ifindex;
  __u8 addr[16];
};
...
// enforced is the set of tap ifindexes enforcement applies to. A tap whose
// ifindex is not a key here is metered but never dropped, so an untagged /
// unpolicied sandbox keeps unrestricted egress. Userspace adds a tap when it
// receives a policy for it (and, in the classic all-taps mode, for every tap).
struct {
  __uint(type, BPF_MAP_TYPE_HASH);
  __type(key, __u32);
  __type(value, __u8);
...
// enforce_iface reports whether enforcement applies to this tap ifindex.
static __always_inline int enforce_iface(__u32 ifindex) {
  return bpf_map_lookup_elem(&enforced, &ifindex) != 0;
}
...
static __always_inline int read_remote(const struct __sk_buff *skb,
  ...
  key->ifindex = skb->ifindex; // the tap this packet crossed — per-VM discriminator
...
// allowed_for reports whether this tap may reach key's remote: a per-tap entry
// (this ifindex, addr) or an all-VMs wildcard entry (ifindex 0, addr).
static __always_inline int allowed_for(struct flow_key *key) {
  if (bpf_map_lookup_elem(&allowed, key))
    return 1;
  struct flow_key wild = *key;
  wild.ifindex = 0;
  return bpf_map_lookup_elem(&allowed, &wild) != 0;
}
...
int from_guest(struct __sk_buff *skb) {
  ...
  if (enforce_on() && enforce_iface(key.ifindex) && !allowed_for(&key))
    return TC_ACT_SHOT; // enforced tap, remote not on its allow-set → drop
  return TC_ACT_OK;
}
